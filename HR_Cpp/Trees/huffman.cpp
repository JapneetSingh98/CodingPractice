...
void decode_huff(node * root, string s) {
    string ans = "";
    ans += decode_helper(root, root, s);
    cout << ans;
}

void decode_helper(node * root, node * cur, string s) {
    if(s.equals("") || cur == NULL) {
        return;

    } else if (root->left == NULL && root->right == NULL) { // leaf node
        ans += (root->data + decode_helper(root, root, s.substr(1));
        return ans;

    } else if (s[0] == '0') {
        return decode_helper(root, root->left, s.substr(1));
    
    } else if (s[0] == '1') {
        return decode_helper(root, root->right, s.substr(1));
